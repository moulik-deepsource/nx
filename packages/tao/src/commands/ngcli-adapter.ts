...
async function runSchematic(
  ...
  schematic: Schematic<FileSystemCollectionDescription,
    FileSystemSchematicDescription>,
...
export async function generate(logger: logging.Logger, root: string, opts: GenerateOptions) {
...
export async function invokeNew(logger: logging.Logger, root: string, opts: GenerateOptions) {
  const fsHost = new virtualFs.ScopedHost(
    new NodeJsSyncHost(),
    normalize(root)
  );
