...
export class Workspaces {
  ...
  readSchematic(collectionName: string, schematicName: string) {
    try {
      const { schematicsFilePath, schematicsJson, normalizedSchematicName } = this.readSchematicsJson(
        collectionName, schematicName
      );
      const schematicsDir = path.dirname(schematicsFilePath);
      const schematicConfig = schematicsJson.schematics[normalizedSchematicName];
      const schemaPath = path.join(
        schematicsDir,
        schematicConfig.schema || ''
      );
      const schema = JSON.parse(
        fs.readFileSync(schemaPath).toString()
      );
      const module = require(path.join(
        schematicsDir,
        schematicConfig.implementation ? schematicConfig.implementation : schematicConfig.factory
      ));
      const implementation = module.default;
      return { schema, implementation };
    } catch (e) {
      throw new Error(`Unable to resolve ${collectionName}:${schematicName}.\n${e.message}`);
    }
  }
  ...
  private readSchematicsJson(collectionName: string, schematic: string) {
    ...
    for (let k of Object.keys(schematicsJson.schematics)) {
      ...
      if (schematicsJson.schematics[k].aliases && schematicsJson.schematics[k].aliases.indexOf(schematic) > -1) {
        normalizedSchematicName = k;
        break;
      }
    }
